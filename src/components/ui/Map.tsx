...
// Coordinates for Brazilian State Capitals
...
export default function Map({
  ...
  useEffect(() => {
    ...
    // Try geocoding city/state via Nominatim
    if (city && state) {
      ...
      fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=1`)
        ...
        .then((data) => {
          if (data && data.length > 0) {
            ...
            const capitalCoords = stateCapitals[uppercaseState] || [-23.55, -46.633]; // Fallback to SP
            setCoords(capitalCoords);
          }
        })
        .catch(() => {
          // Fallback to state capital on error
          const uppercaseState = state.toUpperCase().trim();
          const capitalCoords = stateCapitals[uppercaseState] || [-23.55, -46.633]; // Fallback to SP
          setCoords(capitalCoords);
        });
      ...
      setCoords([-23.55, -46.633]); // Fallback to SP
    }
  ...
  useEffect(() => {
    ...
    if (!mapRef.current) {
      ...
      // Add elegant dark theme tiles (CartoDB Dark Matter fits our dark premium theme perfectly)
    ...
    return () => {
      ...
      if (mapRef.current) {
        // Wait briefly to prevent issues with React 19 strict mode double render
        setTimeout(() => {
          if (mapRef.current) {
