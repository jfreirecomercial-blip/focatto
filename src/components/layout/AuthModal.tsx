...
export function AuthModal({ mode, onClose, onToggle }: AuthModalProps) {
  ...
  // Keep internal mode in sync with parent prop trigger
  useEffect(() => {
    setCurrentMode(mode);
  }, [mode]);
  ...
  return (
    <Modal open onClose={onClose} size="sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        {currentMode !== "forgot-password" && (
          <div className="relative">
            <Input
              ...
              placeholder={currentMode === "register" ? "Mínimo 6 caracteres" : "Sua senha"}
              value={password}
        ...
        {currentMode === "register" && (
          <p className="text-[11px] text-surface-400 text-center mt-3 leading-relaxed">
            Ao se cadastrar, você concorda com nossos{" "}
            <Link href="/termos" className="text-accent hover:underline" target="_blank" rel="noopener noreferrer">
              Termos de Uso
            </Link>
            ,{" "}
            <Link href="/privacidade" className="text-accent hover:underline" target="_blank" rel="noopener noreferrer">
              Política de Privacidade
            </Link>
            ,{" "}
            <Link href="/cookies" className="text-accent hover:underline" target="_blank" rel="noopener noreferrer">
              Política de Cookies
            </Link>{" "}
            e{" "}
            <Link href="/legislacao" className="text-accent hover:underline" target="_blank" rel="noopener noreferrer">
              Legislação
            </Link>
            .
          </p>
      ...
      <div className="mt-4 text-center text-sm text-surface-400">
        {currentMode === "login" ? (
          ...
          <>
            Lembrou da senha?{" "}
            <button onClick={() => setCurrentMode("login")} className="text-accent hover:underline font-medium">
              Voltar para o login
            </button>
          </>
        )}
